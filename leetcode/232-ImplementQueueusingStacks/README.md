...
Hence the total time complexity of the sequence is : $n$ (for push operations) + $2n$ (for first pop operation) + $n-1$ (for pop operations) which is $O(2n)$. This gives $O(2n/2n) = O(1)$ average time per operation.
