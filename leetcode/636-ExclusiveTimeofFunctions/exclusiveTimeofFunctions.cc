...
public:
    vector<int> exclusiveTime(int n, vector<string>& logs)
    {
        ...
        for(auto& entry: logs)
        {
            ...
            getline(ss, token2, ':');
            getline(ss, token3, ':');
            auto item = Log{stoi(token1), token2, stoi(token3)};
            ...
            {
                // assertion proof see README
                assert(item.id == st.top().id);
                int time_elapse = item.timestamp - st.top().timestamp + 1;
                res[item.id] += time_elapse;
                st.pop();
...
using ptr2exclusiveTime = vector<int> (Solution::*)(int, vector<string>&);

void test(ptr2exclusiveTime pfcn)
{
    Solution sol;
    int n = 2;
    vector<string> logs = {
    ...
    vector<int> ans = {3,4};
    assert((sol.*pfcn)(n, logs) == ans);
}

int main()
{
    ptr2exclusiveTime pfcn = &Solution::exclusiveTime;
    test(pfcn);
}
